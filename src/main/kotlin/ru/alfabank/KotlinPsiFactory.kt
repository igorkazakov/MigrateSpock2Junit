 ...
 */
internal class KotlinPsiFactory {
    private var psiFileFactory: KtPsiFactory? = null
    private val acquireCounter = AtomicInteger(0)
    private val initializerLock = Any()

    internal fun getKtPsiFactory(): KtPsiFactory = synchronized(initializerLock) {
        acquireCounter.incrementAndGet()
        return psiFileFactory ?: initializePsiFactory().also {
            psiFileFactory = it
        }
    }
     ...
     */
    private fun initializePsiFactory(): KtPsiFactory {
